...
export default function DeadlineWarningBanner() {
  ...
  useEffect(() => {
    const checkDeadlines = async () => {
      try {
        ...
        let ackInfo = await api.getAppSetting('deadline-ack', { date: '', count: 0, firstTime: 0 });
  ...
  const handleAcknowledge = async () => {
    ...
    await api.saveAppSetting('deadline-ack', ackInfo);
    setIsOpen(false);
  };
    ...
    <div className="w-full bg-rose-50 border border-rose-200 rounded-xl p-4 mb-8 shadow-sm">
      <div className="flex justify-between items-start mb-3">
        ...
        <div className="flex items-center gap-3">
          ...
          <button onClick={() => void handleAcknowledge()} className="text-rose-400 hover:text-rose-600 rounded-lg hover:bg-rose-100 p-1 transition-colors">
            <X size={18} />
          </button>
        </div>
      </div>
