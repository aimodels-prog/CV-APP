...
export default function GeneratedCVs() {
  ...
  const generationModes = values('cv_generation_mode');
    ...
    <div className="space-y-6 max-w-full w-full mx-auto pb-32">
      ...
      <div className="flex items-center gap-4 bg-white p-4 rounded-xl border border-slate-200">
        ...
        <div className="flex items-center gap-2 max-w-xs">
          ...
          >
            {generationModes.map(option => <option key={option.code} value={option.code}>{option.label}</option>)}
          </select>
        </div>
      ...
      {previewCv && (
        <CertificationModal
          ...
          expertName={previewCv.expertName}
          onSave={async (settings) => {
            const updatedCv = { ...previewCv, certification: settings };
            setPreviewCv(updatedCv);
            await api.updateCV(updatedCv);
            setShowCertModal(false);
            handlePreview(updatedCv); // refresh pdf
          }}
        />
      )}
    </div>
