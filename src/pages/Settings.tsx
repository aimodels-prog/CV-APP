...
export default function Settings() {
  ...
  const handleSaveBranding = async () => {
    ...
    try {
      ...
      alert(`Unable to save branding profile: ${error?.message || 'Unknown error'}`);
    } finally {
  ...
  const handleDeleteBranding = async (id: string) => {
    if (await appConfirm("Are you sure you want to delete this branding?", {
      ...
      if (editingBrandingId === id) resetBrandingForm();
      await loadBrandings();
    }
  };
            ...
            <div className="bg-white border border-slate-200 rounded-xl shadow-sm p-6">
              <div className="flex items-start justify-between mb-6">
               ...
               <div className="max-w-3xl space-y-8">
                 {/* Create or edit branding */}
                 <div ref={brandingFormRef} className="bg-slate-50 p-5 rounded-lg border border-slate-200 space-y-4 scroll-mt-6">
                   <div className="flex items-center justify-between gap-4">
                     <h4 className="text-sm font-semibold text-slate-800">
                       {editingBrandingId ? 'Edit Branding' : 'Add New Branding'}
                     </h4>
                     {editingBrandingId && (
                       <button
                         type="button"
                          ...
                          <div className="flex-1 space-y-3">
                            <div className="flex items-center justify-between">
                              ...
                              <div className="flex items-center gap-1">
                                <button
                                  type="button"
                                  onClick={() => handleEditBranding(b)}
                                  className="flex items-center gap-1.5 px-2.5 py-1.5 text-xs font-semibold text-blue-600 hover:bg-blue-50 rounded-md transition-colors"
                                  title={`Edit ${b.name}`}
                                >
                                  <Pencil size={14} />
                                  Edit
                                ...
                                <button
                                  type="button"
                                  onClick={() => handleDeleteBranding(b.id)}
