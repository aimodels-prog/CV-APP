  ...
  return (
    <div className="space-y-6 max-w-full w-full mx-auto pb-32">
      ...
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 mb-8">
        ...
        <div className="flex w-full flex-col gap-2 sm:w-auto sm:flex-row">
          <button
            ...
            className="flex flex-none justify-center items-center gap-2 bg-[#2563eb] hover:bg-blue-700 text-white px-4 py-2.5 rounded-lg font-medium text-sm transition-colors shadow-sm w-full sm:w-[230px]"
          >
            <Plus size={16} />
            Upload Expert
          </button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-6 flex justify-between items-start shadow-sm">
          <div>
            <h3 className="text-[13px] font-semibold text-slate-600 mb-2">Total Matches found</h3>
            <p className="text-[32px] leading-none font-bold text-slate-900">{stats.totalMatches || 0}</p>
          </div>
              ...
              {filteredTenders.length > 0 ? (
                filteredTenders.map((tender) => (
                  <tr key={tender.id} className="border-b border-slate-100 hover:bg-slate-50 transition-colors cursor-pointer" onClick={() => navigate(`/matches?tenderId=${tender.id}`)}>
                    ...
                    <td className="px-6 py-4 text-sm text-slate-500 truncate max-w-[200px]">{(tender.project_sector && Array.isArray(tender.project_sector) && tender.project_sector.length > 0) ? tender.project_sector.join(', ') : (tender.project_sector && typeof tender.project_sector === 'string' ? tender.project_sector : (tender.tender_format || "GEN-X1"))}</td>
