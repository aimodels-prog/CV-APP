import { getCertificationHtml, CertificationSettings } from './certificationHtml';

export function generateCVHtml(expert: any, positionTitle: string = 'Expert', certificationSettings?: CertificationSettings): string {
  if (!expert) return '';
  ...
  const resolvedTitle = positionTitle && !positionTitle.startsWith("pos_") ? positionTitle : (expert.primary_position || "Specialist");
  let html = `
    <div style="font-family: Arial, sans-serif; max-width: 800px; margin: 0 auto; color: #333;">
      <h1 style="text-align: center; color: #1e3a8a; border-bottom: 2px solid #1e3a8a; padding-bottom: 10px;">${fullName}</h1>
      <p style="text-align: center; font-size: 1.1em; color: #64748b;"><strong>Proposed Position:</strong> ${resolvedTitle}</p>
    ...
    expert.employment_history.forEach((h: any) => {
      tableRows += `
        <tr>
          ...
          <td style="border: 1px solid #cbd5e1; padding: 10px; white-space: pre-wrap;">${(() => {
  ...
  if (expert.adequacy_experience && expert.adequacy_experience.length > 0) {
    ...
    expert.adequacy_experience.forEach((item: any) => {
      ...
      if (item.assignment) {
          rows += `<p style="margin: 0;"><strong>Assignment:</strong><br/>${(() => {
              let rawDesc = String(item.assignment || "");
              return rawDesc.split('\n').map(p => {
                  let line = p.trim();
                  if (!line) return "";
                  if (line.startsWith("-") || line.startsWith("•")) {
  ...
  if (certificationSettings && certificationSettings.show !== false) {
    html += getCertificationHtml(fullName, certificationSettings);
  }
